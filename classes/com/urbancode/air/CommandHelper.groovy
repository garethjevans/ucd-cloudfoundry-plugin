...
public class CommandHelper {
    ...
    public int runCommand(def message, def command) {
        return runCommand(message, command, null, false)
    }
     ...
     * @param withBash indicates if the command is a bash script
     */
    public int runCommand(def message, def command, Closure closure, Boolean withBash) {
        command[0] = sanitizeExecutable(command[0])
        pb.command(addShellPrefix(command as String[], withBash))
        println()
        if (message) {
            println(message)
        }
    ...
    private String[] addShellPrefix(String[] command, Boolean withBash) {
      //println "Adding shell prefix: " + command.join(" ")
      String stringCommand = command.join(" ");
      if (withBash == true) {
        stringCommand = "bash " + stringCommand;
      }
      return ["sh", "-c", stringCommand];
    }
